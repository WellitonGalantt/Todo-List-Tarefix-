...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function listAllTask(tasks){
        ...
        tasks[0].forEach(task => {
            ...
            taskItem.innerHTML = `
                ...
                <span class="task-categoria" data-category-id="${category.id}">${category.name}</span>
                <span class="task-status ${status.name === 'Completa' ? 'concluida' : 'pendente' }" data-status-id="${status.id}">${status.name}</span>
    ...
    function createTaskFun(event){
        ...
        const categorySelectIndex = document.getElementById('taskCategory').selectedIndex;
        const statusSelectIndex = document.getElementById('taskStatus').selectedIndex;
        let category_id;
        let status_id;
        // --------------------------
        if( categorySelectIndex !== -1){
            category_id = document.getElementById('taskCategory').options[categorySelectIndex].dataset.categoryId;
        }

        if( statusSelectIndex !== -1){
            status_id = document.getElementById('taskStatus').options[statusSelectIndex].dataset.statusId;
        }
    ...
    async function editTask(id){
        ...
        const categorySelect = document.getElementById("taskCategory").getElementsByTagName('option');
        const statusSelect = document.getElementById("taskStatus").getElementsByTagName('option');
        ...
        createTaskFormEl.addEventListener('submit', editTaskFun);
        
        // const title = elemento.getElementsByClassName("task-title")[0].innerText
        // const idCategory = elemento.getElementsByClassName("task-categoria")[0].dataset.categoryId
        // const idStatus = elemento.getElementsByClassName("task-status")[0].dataset.statusId
        // console.log(idStatus)
        
    }

    function editTaskFun(event){
        event.preventDefault();
        console.log('Editar!')
        createTaskFormEl.removeEventListener('submit', editTaskFun);
        createTaskModalEl.style.display = "none";
        createTaskFormEl.reset();
    }
